// stdDevP calculator https://www.calculator.net/standard-deviation-calculator.html
topSuite("Ext.data.summary.StdDevP", ['Ext.data.Model', 'Ext.Number'], function() {
    var aggregator;

    beforeEach(function() {
        aggregator = new Ext.data.summary.StdDevP();
    });
    ...
    function makeSuite(withRecords) {
        ...
        describe("value", function() {
            ...
            it("should return the correct stdDevP", function() {
                data = [10, 12, 23, 23, 16, 23, 21, 14];
                expect(run()).toBe(5.04);
            });

            it("should return the stdDevP when there are duplicates", function() {
                data = [10, 12, 23, 23, 16, 23, 21, 14, 10, 12];
                expect(run()).toBe(5.28);
            });

            it("should return the stdDevP correctly with negative numbers", function() {
                data = [-8, -11, -14, -4, -2, -17, -6, -9, -5, -12];
                expect(run()).toBe(4.49);
            });
        });
        ...
        describe("property", function() {
            ...
            it("should run on the correct property", function() {
                expect(run('p1')).toBe(6.33);
            });

            it("should run on another property", function() {
                expect(run('p2')).toBe(4.82);
            });
        });
